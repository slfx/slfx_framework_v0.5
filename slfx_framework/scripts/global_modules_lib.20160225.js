$.fn.galleryView = function( opts ){
	var weights, id_index = Array(), url_index = Array(), src_index = Array(), ar_index = Array();

	var defaults = {
		photos	:		null,
		container	: 	null,
		container_id :	'#grid_solve',
		item_classes :	'photo boxer protect',
		doc_title	: 	"brickwall"
	};

	var obj = $.extend( defaults, opts );
	obj.container = $( obj.container_id );

	obj.initialize = function(){
		obj.container.hide();
		// do some initial math and fill some arrays
		$.each( obj.photos, function(index,value){
			return obj.galleryPhotoView( value );
		});

		// index each photo id
		for(var _i = 0; _i < obj.photos.length; _i++) {
			src_index[_i] = obj.photos[_i].src;
			url_index[_i] = obj.photos[_i].url;
			id_index[obj.photos[_i].id] = _i;
		};

		// it doesn't change so only do it once
		weights = obj.photos.map( function( p ){
			return Math.round( p.aspect_ratio * 1000 );
		});

		// map ar to index
		for(var _i = 0; _i < obj.photos.length ; _i++) {
			ar_index[_i] = Math.round( obj.photos[_i].aspect_ratio * 1000 );
		}
		
		var callback = function () { _this = obj; _this.layout(); }
		obj.up( true, callback );
		return;
	};

	obj.up = function( animated, callback ){
		...
		if( obj.isUp() ){
			...
			if( animated ){
				obj.container.fadeIn( 500, callback );
			} else {
				obj.container.show();
				if( typeof callback === "function" ) { callback(obj); }
			}
			obj.layout(1);
			$(window).on( 'resize', obj.debouncedLayout );
			return $(window).on( 'scroll', obj.debouncedLazyLoad );
		}
	};

	obj.isUp = function(){
		return obj.container.is(':visible');
	};

	obj.lazyLoad = function(){
		var container, threshold, viewport_bottom, viewport_top, _this = this;

		threshold = 1500;
		viewport_top = $(document).scrollTop() - threshold;
		viewport_bottom = $(window).height() + $(document).scrollTop() + threshold;

		$( obj.container_id + ' .photo').each( function(index) {

			var bottom = $(this).position().top + $(this).height();
			var id = $(this).attr('id');
			var photo_num = id_index[id];

			if(photo_num == undefined)
				return false;

			if( $(this).position().top < viewport_bottom && bottom > viewport_top ){
				var url;
				var src;

				// the image src
				var src = src_index[photo_num];

				// the url
				var url = url_index[photo_num];
				return obj.load(id, src, url, photo_num);
			} else {
				return obj.unload(id, photo_num);
			}
		});
		$('.boxer').boxer();
	};
	...
	obj.layout = function() {
		var ideal_height, index, partition, photos, row_buffer, rows, summed_width, viewport_width;
		var photos = obj.photos;
		var row_count = 0;

		var init = 0;
		// will a scroll bar be needed?
		if( arguments[0] !== undefined && isMobile == 0 ) {

			init = 0;
		}

		// get all dimensions and row num
		viewport_width = obj.container.width() - init;
		ideal_height = Math.round( $( window ).height() / 2 );
		summed_width = _.reduce( photos, function( sum, p ) {
			return sum += Math.ceil(p.aspect_ratio * ideal_height);
		}, 0 );
		...
		if(partition.length == 0 ) {
			...
			return _.each( photos, function( item ) {
				obj.container.append( item.elem );

				// load it fresh
				item.loaded = false;
				$('.current').attr('id', item.id);
				$('.current').attr('href', url_index[item]);

				resize( $('.current'), Math.round( ideal_height * item.aspect_ratio ), Math.round( ideal_height ) );
				viewport_width += init;
		...
		_.each( partition, function( row ) {
			...
			// plan out the next row
			// go through all photos
			for(var _c = 0; _c < row.length; _c++) {
				var _i = -1;
				while(  ar_index[++_i] != row[_c]
					|| jQuery.inArray( _i, row_used ) > -1
					&&  _i < ar_index.length -1 );
				row_buffer.push(_i);
				row_used.push(_i);
			}

			summed_ars = _.reduce( row, ( function( sum, p ){ return sum += p } ), 0 );

			return _.each( row_buffer, function( item ) {
				obj.container.append( photos[item].elem );

				// load it fresh
				obj.photos[item].loaded = false;
				$('.current').attr('id', obj.photos[item].id);
				$('.current').attr('href', url_index[item]);

				resize( $('.current'), Math.floor( viewport_width / summed_ars * obj.photos[item].aspect_ratio * 1000 ), Math.floor( viewport_width / summed_ars * 1000 ) );
	...
	obj.debouncedLazyLoad = _.debounce( function() { return obj.lazyLoad(); }, 100, false );

	obj.debouncedLayout = _.debounce( function() { return obj.layout();}, 100, false );

	function resize( target, width, height ){
		var newWidth = width - parseInt(target.css('marginLeft')) * 2;
		var newHeight = height - parseInt(target.css('marginTop')) * 2;
		//alert('height is ' + newHeight);
		target.css({ width: parseInt(newWidth), height: parseInt(newHeight) } );
		target.top = target.offset.top;
		target.bottom = target.top + target.height();
		return;
	};

	obj.load = function( target, src, url, index){
		var url;

		$('#' + target).css('background-color','#666');

		if( obj.photos[index].loaded === false && obj.photos[index].loaded ){
			return obj.photos[index].loaded = true;
		}

		$('#' + target).css('backgroundImage',"url(" + src.replace(' ','%20') + ")");
		//console.log(url);
		return obj.photos[index].loaded = true;
	};

	obj.unload = function(target, index) {
		$('#' + target).css('background-color','#dcdcdc');
		$('#' + target).css('background-image',"");
		obj.photos[index].loaded = false;
		return true;
	};

	obj.galleryPhotoView = function( photo_obj ) {
		photo_obj.className = 'photo';
		photo_obj.elem = "<a class=\"" + obj.item_classes + " current\" data-gallery=\"gallery\" href=\"\"><div><div></div></div></a>";
		photo_obj.loaded = false;
		return photo_obj;
	}
